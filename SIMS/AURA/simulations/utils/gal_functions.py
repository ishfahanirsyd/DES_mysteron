...
def ozdes_efficiency(dir='/priv/debass/software/DES/SIMS/efficiencies'):
    ...
    for f in fs:
        for y in ys:
            eff = pd.read_csv(os.path.join(dir,'eff_%s_Y%s.dat'%(f,y)),sep=' ',skipinitialspace=True)
            #print(eff.columns)
            #print(eff['r_obs'])
            #print(eff['obs_gr_auto'])    
            #print(eff['HOSTEFF'])
...
def interpolate_zdf_oldver(zdf,marr):
    ...
    #print('line2 gb',gb.columns)
    gb.reset_index(drop=True,inplace=True)
    #print('line3 gb',gb.columns)

    # Ensure the 'mass' column is numeric
    #zdf['mass'] = pd.to_numeric(zdf['mass'], errors='coerce')

    # Filter numeric columns
    #numeric_cols = zdf.select_dtypes(include=[np.number]).columns

    # Group by mass bins and aggregate using mean
    #gb = zdf.groupby(pd.cut(zdf['mass'], bins=marr))[numeric_cols].agg(np.mean)

    # Drop rows with NaN values in the 'mass' column
    #gb.dropna(subset=['mass'], inplace=True)
