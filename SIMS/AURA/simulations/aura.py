...
# masih belum nemu aura_dir dimana
aura_dir = os.environ['DESSIMS']
idx = pd.IndexSlice
...
class Sim(SN_Model):
    def __init__(self,conf_path,cosmo='default'):
        ...
        if hostlib_fn[0] == '$':
            i = 1
            varname = ''
            while i < len(hostlib_fn) and hostlib_fn[i] != '/':
                varname += hostlib_fn[i]
                i += 1
            env_value = os.environ.get(varname)
            if env_value is None:
                raise ValueError(f"Environment variable '{varname}' is not set.")
            self.hostlib_fn = env_value + hostlib_fn[i:]
        else:
            self.hostlib_fn = hostlib_fn

        # load hostlib data
        self.flux_df = self._load_flux_df(self.hostlib_fn)
        #calculate rate by using shechter function (phi, equation SMZ)
        ...
        if cosmo=='default':
            ...
            self.cosmo=cosmo
        # get the name of the model function from the config file
        self._get_funcs()

    def _get_config(self,conf_path):
        with open(conf_path,'r') as f:
            return yload(f)
    #fn is path to the file, not used again
    def _load_flux_df_old(self,fn):
        ...
        for col in full_df.columns:
            try:
                ...
                print('',end='')
        return full_df
    # load flux data and proccess it into pandas dataframe
    ...
    def _calculate_absolute_rates(self):
        ...
        #self.flux_df['phi']= self.flux_df[['z','mass','SF']].apply(lambda x: schechter(x.iloc[0],np.log10(x.iloc[1]),x.iloc[2]),axis=1)
        #get expected number of SN Ia for each strech parameter value
        ...
        # the expected number of SNe Ia 
        self.flux_df['N_total'] = self.flux_df['pred_rate_total']*self.flux_df['phi']
        #mksdnprint('N_total', self.flux_df['N_total'])
    ...
    def _get_z_dist(self,z_vals,n=50,frac_low_z=0.0,zbins=[]):
        ...
        '''
        if len(zbins)==0:
            zbins=[0.15, 0.3, 0.45, 0.6, 0.75, 0.9, 1.05, 1.2]
        ...
        #number of samples in each redshift bin
    ...
    # simulate the SN based on the redshift and number of samples
    def sample_SNe(self,z_arr,n_samples_arr,save_df=True,savepath='default'):
        ...
        for z, n in zip(z_arr, n_samples_arr):
            ...
            self.sim_df = pd.concat([self.sim_df, self._sample_SNe_z(z, n)], ignore_index = True)
            #sim_dfs.append(self._sample_SNe_z(z, n))

        # Concatenate all DataFrames into one
       # self.sim_df = pd.concat(sim_dfs, ignore_index=True)

        #for z,n in zip(z_arr,n_samples_arr):
            #self.sim_df = self.sim_df._append(self._sample_SNe_z(z,n))
    ...
    #in each grid, simulate the SNe based on the redshift and number of samples
    def _sample_SNe_z(self,z,n_samples):
        ...
        #select subset of galaxies at given redshift and create a copy
        #self.multi_df is a multi index dataframe with z, mass and Av as the index
        z_df = self.multi_df.loc['%.5f' % z].copy()
        #print(z_df.head())
        #print('N_total:', z_df['N_total'], z_df['U_R'], 'end')
        # Filter rows where N_total == 0
        #z_df_zero = z_df[z_df['N_total'] == 0]

        # Print the filtered rows
        #if not z_df_zero.empty:
            #print(z_df_zero[['N_total', 'U_R']])
            #print('--- End of zero N_total entries ---')
        #print('z_df_1', z_df.columns)
        # remove the galaxies that have no SNe
        # Remove galaxies with no SNe, when N_total is 0
        ...
        #resample galaxy data by using the unique values of Av, then selects correspoding subset of galaxies and interpolate their data into a log linear mass grid
        resampled_df = pd.DataFrame()
        
        #print('resampled_df',resampled_df.columns)
       
        marr= np.logspace(6,11.6,100)
        #print('z_df_c', z_df.columns)
        # in each grid (which has multi index), choses the unique values of Av and then selects the corresponding subset of galaxies
        for av in z_df.Av.unique():  
            av_df =z_df.loc[idx[:, '%.5f'%av, :]]
            # interpolate zdf into log linear mass grid
            av_df = interpolate_zdf(av_df,marr)
            #print('av_df', av_df.columns)
            resampled_df = pd.concat([resampled_df, av_df], ignore_index = True)
            #resampled_df = resampled_df.append(av_df)
        #print('resampled_df_c',resampled_df.columns)
        #set multi index for the resampled dataframe
        ...
        #create a list of unique galaxy masses
        ...
        # sample extinction values from the galaxy mass distribution (randomly)
        m_av0_samples = [(m, '%.5f' % (np.random.choice(new_zdf.loc[m].Av.values))) for m in m_samples]
        #print('m_av0_samples',m_av0_samples.columns)
        ...
        # calculate the age distribution of SNe for each galaxy mass
        #group galaxy by mass bins and calculate the age distribution for each group
        # we use age distribution from make_hostlib (.dat files)
        hostlib_fn = self.config['hostlib_fn']
        if hostlib_fn[0] == '$':
            i = 1
            varname = ''
        ...
        #for Band in ['U', 'B', 'V', 'R', 'I']:
            #args['%s'%Band] = gals_df['%s'%Band].values
        ...
        #print(z_sim_df.columns)
        z_sim_df['z'] = z
        #print('z_sim_df',z_sim_df['ssfr'])
        return z_sim_df
