...
from AURA.simulations.synspec2 import SynSpec, phi_t_pl
import argparse
...
def parser():
    ...
    parser.add_argument('-o','--output',help='Output directory',default='test_oII')
    ...
    parser.add_argument('-zh','--zhi',help='Redshift upper end',default=1.0,type=float)
    ...
    parser.add_argument('-na','--n_av',help='Av step',default=15,type=int)
    ...
    parser.add_argument('-tr','--time_res',help='SFH time resolution',default=3,type=int)
...
#        bc03_dir = '%s/templates/bc03/bc03_ssp_templates_generated/' % os.environ["DESSIMS"]
        ...
        s = SynSpec(library='PEGASE',template_dir = '-',neb=args.neb)
    ...
    for fn in glob.glob(os.path.join(
        outputDir, 
        'all_model_params_quench_BC03_z*_*_av*_*_rv_rand_full_age_dists_neb_U-2.00_res_5_beta_1.14_*_*_sdss_u_r.h5')):
