...
def parser():
    ...
    parser.add_argument('-d','--inputDir',help='Directory with hostlib files', default=None)
    parser.add_argument('-o','--outputFile',help='Output filename', default=None)
...
def main(args):
    ...
    df_list = []
    for fn in tqdm(glob.glob(os.path.join(os.environ['DESSIMS'],'hostlib',args.inputDir,'*av0.00_1.50*.h5'))):
        df = pd.read_hdf(fn)
        df_list.append(df)
    ...
    # Save to HDF5
    outputFile=os.path.join(os.environ['DESSIMS'],'hostlib',args.inputDir,args.outputFile)
    full_df.to_hdf(outputFile, key='main')

    # Rename .dat files
    for fn in tqdm(glob.glob(os.path.join(os.environ['DESSIMS'],'hostlib',args.inputDir,'SN_ages/*'))):
