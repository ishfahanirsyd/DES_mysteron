...
def parser():
    ...
    parser.add_argument('-i','--input',help='Filename for input SFHs', default='%s/SFH_mpi/shark/sfr_massformed_keys_multiplied.h5'  % (os.environ["mass_assembly"]))
...
def sed_worker(worker_args):
    ...
    try:
        ...
        mtot=sfh_iter_df['m_total'].iloc[-1]
        ...
        pred_rate_x1hi =np.sum(sfh_iter_df['sfr']*dtd_x1hi)
        dtd_x1lo = phi_t_pl(ages,0.04,beta_x1lo,norm_x1lo)
        pred_rate_x1lo =np.sum(sfh_iter_df['sfr']*dtd_x1lo)
        dtd_total =phi_t_pl(ages,0.04,-1*args.beta,dtd_norm)
        SN_age_dist = sfh_iter_df['sfr']*dtd_total
        ...
        mwsa = np.average(sfh_iter_df['stellar_age'],weights=sfh_iter_df['sfr']/mtot)
        ...
        arr[:,1] = SN_age_dist
        #print(arr)
        np.savetxt('%s/SN_ages/all_model_params_quench_%s_z_%.5f_rv_rand_full_age_dists_neb_U%.2f_res_%i_beta_%.2f_%.1f.dat' % (outputDir,args.templates, z, args.logU, args.time_res, args.beta, tf),arr)
...
#    aura_dir = '%s/templates/' % os.environ["DESSIMS"]
    ...
    ordered_keys = np.sort([float(x.strip('/')) for x in store.keys()])
    ...
    for z in tqdm(z_array):
        ...
        for counter,tf in enumerate(ordered_keys[::-1][np.arange(0,len(ordered_keys),args.time_res)]):   # Iterate through the SFHs for galaxies of different final masses
            ...
            print('Reading SFH for final mass formed %.1f' % tf)
            sfh_df = store['%5.0f' % tf]
